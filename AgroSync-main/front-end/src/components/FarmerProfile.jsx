const handleUpdateAgreementStatus = async (agreementId, status) => {
    if (status === 'Accepted' && (!signaturePad.current || signaturePad.current.isEmpty())) {
        alert('Please sign the agreement before accepting');
        return;
    }

    const signatureData = status === 'Accepted' ? signaturePad.current.toDataURL() : null;
    console.log('Farmer signature data:', signatureData ? signatureData.substring(0, 50) + '...' : 'No signature');

    try {
        const response = await fetch(`http://localhost:3000/agreements/${agreementId}/status`, {
            method: 'PUT',
            headers: {
        ...
        if (data.status) {
            ...
            alert(data.message || `Failed to ${status.toLowerCase()} agreement`);
        }
    } catch (error) {
        console.error('Error updating agreement status:', error);
        alert('Error updating agreement status. Please try again.');
    }
};
